package config

import (
	"fmt"
	"net/url"
)

/*
Go 言語で構成ファイルを管理し、WebSocket または HTTP 通信のための URL を生成するための設定を定義しています。config パッケージ内で、アプリケーションが使用する設定 (Cfg 構造体) や URL を生成するための関数 (GetBaseURL) が定義されています。

WebSocketまたはHTTP通信のために、適切なURLを生成するための構成データを管理する仕組みを提供しています。特に、通信がセキュアかどうかに応じて、プロトコル（HTTP/HTTPS、WS/WSS）を動的に決定します。また、暗号化されたデータやバージョン管理に関連する情報（ConfigBuffer や COMMIT）も管理されています。
*/

/*
Secure: 通信が HTTPS/WSS であるかを示すフラグ（true ならセキュア、false なら非セキュア）。
Host: 接続先のホスト名または IP アドレス。
Port: 接続先のポート番号。
Path: 接続先のパス。
UUID: クライアントやデバイスを識別するための UUID。
Key: 通信の暗号化に使用するキー。

*/
type Cfg struct {
	Secure bool   `json:"secure"`
	Host   string `json:"host"`
	Port   int    `json:"port"`
	Path   string `json:"path"`
	UUID   string `json:"uuid"`
	Key    string `json:"key"`
}

// Localhost for my development only.
// Shall be commented out when development is done.
//var ConfigBuffer = ""

//ConfigBuffer: 通信に使用するバッファデータです。このコードではデフォルトの設定として、0x19 で埋められたデータが使用されていますが、コメント部分にあるように、開発中には別の値が使用されることもあります。このバッファはおそらく暗号化された設定データを格納するためのものです。
// None
var ConfigBuffer = ""

//COMMIT: 現在のコミットハッシュを格納する変数です。バージョン管理や自己アップグレードのために使用されることがあります。ここでは初期値として空文字列が設定されています。
// COMMIT means this commit hash, help to identify version and self upgrade.
var COMMIT = ``

//Config: Cfg 構造体のインスタンスです。アプリケーション全体で使用する設定を保持します。
var Config Cfg

/*
GetBaseURL: この関数は、WebSocket（WS）または HTTP のベース URL を構築して返します。引数 ws が true の場合は WebSocket 用の URL を、false の場合は HTTP 用の URL を生成します。
URL のスキームは、Config.Secure に基づいて https/http もしくは wss/ws が選ばれます。
Host と Port は Config 構造体に設定された値が使用され、Path には接続するパスが設定されます。
*/
func GetBaseURL(ws bool) string {
	/*
		GetBaseURL: ws 引数に応じて、HTTP/HTTPS または WS/WSS 用のベースURLを生成します。
		ws が true の場合、WebSocket用のURL（ws か wss）を返します。
		ws が false の場合、HTTPまたはHTTPSのURLを返します。
		Config.Secure が true なら、HTTPS または WSS プロトコルを使用し、false なら HTTP または WS プロトコルを使用します。
		Host と Port、および Path は Config 変数の値を使用してURLを構築します。
	*/
	baseUrl := url.URL{
		Host: fmt.Sprintf(`%v:%d`, Config.Host, Config.Port),
		Path: Config.Path,
	}
	if ws {
		if Config.Secure {
			baseUrl.Scheme = `wss`
		} else {
			baseUrl.Scheme = `ws`
		}
	} else {
		if Config.Secure {
			baseUrl.Scheme = `https`
		} else {
			baseUrl.Scheme = `http`
		}
	}
	return baseUrl.String()
}
